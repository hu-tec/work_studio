...
export interface MemberData {
  ...
  isGroup: boolean;
  groupName: string;
  businessNumber: string;        // 사업자번호 (기업회원)
  joinDate: string;
  ...
  expertRating: number;           // 0.0 ~ 5.0
  expertBankName: string;
  expertAccountNumber: string;
  expertSpecialty: string;        // 학력/경력 요약
  ...
  languagePair: string;           // 예: '한→영'
  matchingStatus: MatchingStatus;
  matchedExpertName: string;
  requestDate: string;
  completionDate: string;
  workUnit: string;               // 예: '500자', '1시간'
  finalServiceAmount: number;
  fileCount: number;
  aiUsageCount: number;
  editorUsageCount: number;
  usedAI: string;                 // 사용 AI 프로그램
  usedEditor: string;             // 사용 에디터
  ...
  certificateStatus: string;      // '발급완료' | '미발급' | '배송중'

  // ── 5. 결제/정산 (Payments) ──
  paymentStatus: '결제완료' | '미결제' | '환불' | '구독중' | '';
  paymentCategory: PaymentCategory;
  paymentAmount: number;          // 이용금액
  discountRate: number;           // 할인율 %
  finalPaymentAmount: number;     // 최종결제액
  paymentDate: string;
  ...
  subscriptionBenefit: string;    // 예: '월 5,000자, AI 무제한'

  // ── 6. 정산 (전문가용) ──
  settlementAmount: number;
  settlementRate: number;         // 정산비율 %
  settlementDate: string;
  settlementStatus: string;       // '정산완료' | '정산대기' | '미정산'
...
function generateMember(id: number): MemberData {
  ...
  return {
    ...
    finalPaymentAmount: 0, // calculated below
...
function applyDiscount(m: MemberData): MemberData {
  const discount = m.paymentAmount * (m.discountRate / 100);
  return { ...m, finalPaymentAmount: m.paymentAmount - discount };
}

export const mockMembers: MemberData[] = Array.from({ length: 45 }, (_, i) => applyDiscount(generateMember(i + 1)));
